...
const JBREAKS=[-61,9,38,199,426,686,756,818,1111,1181,1210,1635,2060,2097,2192,2262,2324,2394,2456,3178];
const jdiv=(a,b)=>~~(a/b);
const jmod=(a,b)=>a-~~(a/b)*b;
function jalCalCore(jy){
  const gy=jy+621;
  let leapJ=-14,jp=JBREAKS[0],jm=0,jump=0;
...
function jalaliOf(d){return g2j(d.getFullYear(),d.getMonth()+1,d.getDate());}
...
const DOW_FA=['یکشنبه','دوشنبه','سه‌شنبه','چهارشنبه','پنجشنبه','جمعه','شنبه'];      // indexed by Date.getDay()
...
Auckland|New Zealand|NZ|Pacific/Auckland|Asia Pacific`.split('\n').map(x=>{const[city,country,code,zone,region]=x.split('|');return{city,country,code,zone,region}});
...
const C={
  en:{
    ...
    label:'WORLD CLOCKS',title:'Time, everywhere.',
    desc:'Search cities and countries. Accurate, live, and always in sync.',
    search:'Search city or country…',more:'Show more clocks ＋',
    calLabel:'SOLAR CALENDAR',calTitle:'Every day, in both calendars.',
    calDesc:'Live Persian (Jalali) and Gregorian months, kept in step with NTP-synchronized time.',
    today:'TODAY',tTime:'LOCAL TIME · NTP SYNC',
    calJ:'Persian (Jalali)',calG:'Gregorian',
    src:'Source',srcAuto:'Auto · NTP network',srcLocal:'Local clock only',syncNow:'Sync now',
    made:'MADE FOR REAL LIFE',sync:'Built to keep you in sync.',
  ...
  fa:{
    ...
    label:'ساعت‌های جهان',title:'زمان، در هر نقطه.',
    desc:'شهرها و کشورها را جست‌وجو کنید؛ دقیق، زنده و همیشه هماهنگ.',
    search:'جست‌وجوی شهر یا کشور…',more:'نمایش ساعت‌های بیشتر ＋',
    calLabel:'تقویم شمسی',calTitle:'هر روز، در دو تقویم.',
    calDesc:'ماه‌های شمسی (جلالی) و میلادی به‌صورت زنده، هماهنگ با زمان شبکه (NTP).',
    today:'امروز',tTime:'زمان محلی · همگام NTP',
    calJ:'شمسی (جلالی)',calG:'میلادی',
    src:'منبع',srcAuto:'خودکار · شبکه (NTP)',srcLocal:'فقط ساعت محلی',syncNow:'همگام‌سازی',
    made:'برای زندگی واقعی',sync:'برای هماهنگ ماندن ساخته شده.',
...
function parseTimeValue(s){
  ...
  if(m[8]&&m[8]!=='Z'){
    ...
    const oh=+m[8].slice(1,3)||0,om=+m[8].slice(-2)||0;
...
const TIME_SOURCES=[
  {
    ...
    async get(){
      ...
      if(!r)return null;
      const line=r.text.split('\n').map(x=>x.trim()).find(l=>l.startsWith('ts='));
      if(!line)return null;
...
function updateSyncUI(){
  ...
  if(!el)return;
  const dot=$('#syncDot');
  if(dot)dot.className='dot '+(syncState.status==='ok'?'ok':syncState.status==='syncing'?'busy':'warn');
  const stamp=time(new Date(now()),LOCAL_TZ,true);
  if(syncState.status==='syncing'){
    el.textContent=lang==='fa'?'در حال همگام‌سازی با سرور زمان…':'Syncing with time server…';
  }else if(syncState.status==='ok'){
    const rtt=Math.round(syncState.rtt);
    const off=Math.round(offsetMs);
    if(lang==='fa'){
      const diff=off===0?'ساعت محلی دقیق است':(off>0?`ساعت محلی ${toFa(off)} ms عقب است`:`ساعت محلی ${toFa(-off)} ms جلوتر است`);
    ...
    el.textContent=lang==='fa'?'ساعت محلی دستگاه (همگام‌سازی با شبکه ناموفق)':'Local device clock (network sync unavailable)';
  }
}
/* ================= /NTP ================= */
...
function renderCal(){
  ...
  const n=calSys==='j'?jalMonthLen(calY,calM):new Date(Date.UTC(calY,calM,0)).getUTCDate();
...
function renderToday(){
  ...
  const chip=$('#calChip');
  if(chip)chip.textContent=(lang==='fa'?'تقویم: ':'Calendar: ')+(calSys==='j'?C[lang].calJ:C[lang].calG);
  if(calSys==='j'){
    $('#calTodayMain').textContent=`${num(j[2])} ${J_MONTHS[lang][j[1]-1]} ${num(j[0])}`;
    $('#calTodaySub').textContent=`${refG}${num(t.getDate())} ${G_MONTHS[lang][t.getMonth()]} ${num(t.getFullYear())}`;
  }else{
    $('#calTodayMain').textContent=`${num(t.getDate())} ${G_MONTHS[lang][t.getMonth()]} ${num(t.getFullYear())}`;
    $('#calTodaySub').textContent=`${refJ}${num(j[2])} ${J_MONTHS[lang][j[1]-1]} ${num(j[0])}`;
  }
...
function render(){
  ...
  const tickerHTML=P.slice(0,8).map(p=>`<span>${flag(p.code)} ${localName(p.city)} · ${time(d,p.zone)}</span>`).join('');
  const ticker=$('#ticker');
  if(ticker&&!ticker.firstElementChild?.classList.contains('marquee-track')){
...
function translate(){
  ...
  document.querySelectorAll('[data-i]').forEach(e=>{
    if(e.dataset.i&&t[e.dataset.i]!==undefined)e.textContent=t[e.dataset.i];
  });
...
if(typeof module!=='undefined'&&module.exports){
  module.exports={
    g2j,j2g,g2d,d2g,j2d,d2j,jalCal,jalMonthLen,jalLeap,parseTimeValue,TIME_SOURCES,
    doSync,renderCal,render,renderToday,translate,setCalSys,calNav,initCal,updateSyncUI,
    J_MONTHS,G_MONTHS,DOW_FA,DOW_EN,
    state:()=>({lang,region,limit,q,calSys,calY,calM,offsetMs,syncState,syncing}),
    setLang:l=>{lang=l;},
