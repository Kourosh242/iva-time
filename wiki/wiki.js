...
const pages = [
  'Home','Quick-Start','Installation','World-Clocks',
  'Calendar-System','NTP-Synchronization','Bilingual-Support',
  'Architecture','API-Reference','Adding-New-Cities',
...
function inline(text) {
  return text
    ...
    .replace(/!\[([^\]]*)\]\((\.\.[^)]+|[^)]+\.(svg|png|jpg|webp|gif))\)/g,
      (_, alt, src) => {
        const resolved = src.startsWith('../') ? src.replace('../', '../') : src;
...
function markdown(source) {
  ...
  for (let idx = 0; idx < lines.length; idx++) {
    ...
    if (hm) {
      ...
      const id = hm[2].trim().toLowerCase().replace(/[^\w\u0600-\u06FF]+/g, '-');
    ...
    if (/^<div\s/i.test(raw.trim())) { flushList(); flushTable(); flushBlockquote(); out += raw; continue; }
    if (/^<\/div>/i.test(raw.trim())) { out += raw; continue; }
...
nav.innerHTML = pages.map(p =>
  `<a href="#${p}" data-page="${p}">${titles[p]}</a>`
).join('');
...
async function load(name) {
  ...
  main.innerHTML = '<div class="loading"><span></span><span></span><span></span></div>';
...
/* ── Hash routing ────────────────────────────────────────────── */
window.addEventListener('popstate', () => load(location.hash.slice(1) || 'Home'));
